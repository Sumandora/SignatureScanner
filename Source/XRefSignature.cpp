...
FLATTEN const std::byte* SignatureScanner::XRefSignature::optimized_next(const std::byte* it, const std::byte* end) const
{
	for (; it != end; it++)
...
FLATTEN const std::byte* SignatureScanner::XRefSignature::optimized_prev(const std::byte* it, const std::byte* end) const
{
	for (; it != end; it--)
		if (does_match(it, end))
			return it;
