...
void testByteSignatures()
{
	...
	void* signature = malloc(signature_scanner.size_of.byte_signature);
	...
	uintptr_t* ptr = NULL;
	size_t count;
	void* newSignature = malloc(signature_scanner.size_of.byte_signature);
...
const char* testStringSignatures(void* baseAddress)
{
	...
	void* signature = malloc(signature_scanner.size_of.string_signature);
...
{
	void* signature = malloc(signature_scanner.size_of.xref_signature);
