...
int main()
{
	...
	auto signature = SignatureScanner::Signature("ab f6 7c fa ?? 23 e2 4d 6d ?? d3 f5 f4 46 ce ?? 27 c1 23 49 ec db 85");

	void* hit = signature.FindNextOccurrence(byte_array_hex);
	...
	signature = SignatureScanner::Signature("87 98 60 27 ?? ?? 18 90 c5");

	hit = signature.FindLastOccurrence(hit);
	...
	std::vector<void*> hits = SignatureScanner::Signature("5a").FindAllOccurrences(byte_array_hex, reinterpret_cast<char*>(byte_array_hex) + 512);
