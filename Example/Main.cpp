...
void TestByteSignatures()
{
	...
	void* hit = signature.Next<void*>(byte_array_hex, byte_array_hex + sizeof(byte_array_hex));
	...
	hit = signature.Prev<void*>(hit, byte_array_hex);
	...
	std::vector<void*> hits = SignatureScanner::ByteSignature("5a").All<void*>(byte_array_hex, byte_array_hex + sizeof(byte_array_hex));

	printf("0x5A has %zu hits\n", hits.size());
	assert(hits.size() == 2);
}

void TestStringSignatures()
{
	...
	dlclose(handle);
	const char* string2 = signature.Next<const char*>(linkMap->l_addr);
	printf("'%s' = '%s'\n", string, string2);
